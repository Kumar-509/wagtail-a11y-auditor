...
class ImageAltTextRule(AuditorRule):
    """Audit rule to ensure all images in Wagtail pages have alt text."""

    def check(self, page):
        """
        Check if image blocks in the page's body have alt text.
        
        Args:
            page: A Wagtail Page object with a 'body' StreamField.
        
        Yields:
            Issue: An accessibility issue if an image lacks alt text.
        """
        if not hasattr(page, 'body') or not page.body:
            return  # Skip if no body or body is empty

        for block in page.body:
            if block.block_type == "image":
                # Handle cases where block.value might not be a dict or alt is missing
                alt_text = getattr(block.value, 'alt', None) if hasattr(block.value, 'alt') else block.value.get('alt') if isinstance(block.value, dict) else None
                if not alt_text:
                    yield Issue(
                        severity="error",
                        message="Image missing alt text",
                        explanation="Alt text is required for screen readers per WCAG 2.1, 1.1.1 (Non-text Content).",
...
Example Workflow:
1. Editor adds an image block without alt text.
2. Sidebar updates: “Error: Image missing alt text” with a “Fix Now” button.
3. Editor clicks, enters alt text, and the issue clears instantly.
...
- Week 5-6 (50h): CLI & Docs
  ...
  - Milestone: CLI functional, initial docs ready.
...
- Week 11 (30h): Testing & Refinement
  ...
  - Milestone: Beta release on PyPI, stable functionality.
...
5. About Me
I’m Kumar K, a 21-year-old developer from Hyderabad, Telangana, India (UTC+05:30), with a passion for accessible web design and open-source. I’m a 3rd-year Computer Science undergrad at CMR Technical Campus (CMRTC) and have been coding in Python for 2 years.
...
- Open-Source: Contributed a bug fix to a Django app (GitHub PR #123, 2024).
...
Motivation:
I want to make the web more inclusive, and Wagtail’s community-driven ethos inspires me. GSoC is a chance to grow my skills, collaborate with experts, and contribute something meaningful to a tool I love. Accessibility matters deeply to me because a close friend relies on screen readers, fueling my drive for this project.
